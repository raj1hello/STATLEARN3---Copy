      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Global monitoring of all trainers across organizations.
          </p>
        </div>
      </div>
                ...
                filtered.map((t: any) => (
                  <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <Badge variant={t.status === 'active' ? 'emerald' : 'slate'} size="sm" className="capitalize">
                        {t.status}
                      </Badge>
                    </td>
                    <td className="px-5 py-3 text-right text-slate-500 text-xs">
                      {t.lastActive ? formatDate(t.lastActive) : "-"}
                    </td>
                  </tr>
                ))
              )}
