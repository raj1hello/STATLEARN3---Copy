...
async function debugLoginModal() {
  ...
  console.log('Checking DOM buttons on landing page...');
  const buttonsInfo = await cdp.send('Runtime.evaluate', {
    expression: `
  ...
  console.log('Buttons found:', buttonsInfo.result.value);
  ...
  const clickRes = await cdp.send('Runtime.evaluate', {
    expression: `
      (() => {
        ...
        if (!loginBtn) return 'LOGIN_NOT_FOUND';
        loginBtn.click();
  ...
  console.log('Checking modal DOM elements...');
  const modalInfo = await cdp.send('Runtime.evaluate', {
    expression: `
      (() => {
        ...
        return { hasDialog: !!dialog, inputs, buttons };
      })()
    `,
    returnByValue: true
  });
