        ...
        if (opcode === 1) {
          try {
            ...
            if (data.id && callbacks.has(data.id)) {
              ...
              console.log('[NET RES]', data.params.response.status, data.params.response.url);
...
async function verifyLoginOnLocal() {
  ...
  const hydrationCheck = await cdp.send('Runtime.evaluate', {
    expression: `
      (() => {
        ...
        return { reactKeys, nextData: !!window.__NEXT_DATA__ };
      })()
    `,
    returnByValue: true
  });
  console.log('Local hydration check:', hydrationCheck.result.value);
  ...
  const modalCheck = await cdp.send('Runtime.evaluate', {
    expression: `
      (() => {
        ...
        return {
          hasDialog: !!dlg,
          inputs: Array.from(document.querySelectorAll('input')).map(i => ({ type: i.type, val: i.value, ph: i.placeholder })),
  ...
  console.log('Modal after click on LOCAL:', modalCheck.result.value);

  cdp.close();
  process.exit(0);
}

verifyLoginOnLocal().catch(err => {
  console.error(err);
  process.exit(1);
});
